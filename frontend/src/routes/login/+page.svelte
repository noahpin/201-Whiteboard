<script>
  ...
  import { page } from "$app/state";
	...
	console.log($state.snapshot(page).url.split("?redirect=")[1]);
	...
	async function handleSubmit(event) {
		...
		try {
			const response = await fetch(
				`${PUBLIC_LOCALHOST_URL}/whiteboard201/login/verify`,
				{
					// Java dev server
					method: "POST",
        ...
        if(
          $state.snapshot(page).url.split("?redirect=")[1] !== undefined &&
          $state.snapshot(page).url.split("?redirect=")[1] !== ""
        ) {
          goto("edit-whiteboard/"+ page.url.href.split("?redirect=")[1]);
        } else {
          goto("/");
        }
